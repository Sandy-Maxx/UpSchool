 ...
 *
 * This tests the complete authentication flow using fetch API with mocked responses
 * to simulate real API behavior in a controlled test environment.
 */
...
const mockFetch = jest.fn((url: string, options: any = {}) => {
  ...
  // Handle special cases first before JSON parsing
  ...
  // Login endpoint
  if (url === `${API_BASE_URL}${AUTH_PREFIX}/login/` && method === 'POST') {
    const { username, password, email } = body;

    // Validate required fields (Django-style validation)
    if (!username && !email) {
      return createMockResponse(400, {
        success: false,
        error: {
    ...
    // Successful login response (Django REST Auth format)
  ...
  // Refresh endpoint
  if (url === `${API_BASE_URL}${AUTH_PREFIX}/refresh/` && method === 'POST') {
    const { refresh } = body;
  ...
  // Logout endpoint
  if (url === `${API_BASE_URL}${AUTH_PREFIX}/logout/` && method === 'POST') {
...
describe('Authentication Integration - Production Security Tests', () => {
  ...
  describe('Security Attack Prevention', () => {
    ...
    test('should enforce rate limiting simulation', async () => {
      // This would be expanded with actual rate limiting in production
  ...
  describe('Response Format Validation', () => {
    ...
    test('should include proper security headers in responses', async () => {
      ...
      // Verify the response structure supports them
